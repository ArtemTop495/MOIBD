...
# ========== КОНВЕРТАЦИЯ ВАЛЮТЫ ==========
# Предположим, что модель была обучена на данных в турецких лирах (TRY)
# Конвертируем в рубли по курсу (примерно 1 TRY = 3 RUB)
...
async def predict_price(features: PropertyFeatures):
    ...
    try:
        # Вычисляем дополнительные признаки для лучшего предсказания
...
async def calculate_example():
    ...
    # Варианты с разной площадью для демонстрации
    examples = []
